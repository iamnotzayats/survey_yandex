...
try:
    ...
    if random_answer_trade_union == 'Да':
        ...
        list_role_trade_union = ['рядовой член профсоюза', 'профсоюзный активист (участник и/или организатор мероприятий)',
                                 ...
                                 'член выборного органа (член профкома/цехкома), член комиссии профкома', 'штатный работник профсоюзной организации']
        ...
        # choise why member
        list_why_member = ['по убеждению', 'по традиции (семейные, трудовые)', 'из чувства солидарности', 'в расчете на материальную помощь',
                           'в расчете на защиту трудовых прав', 'для получения социальных льгот', 'для участия в профсоюзных мероприятиях',
        ...
        #
        list_prioritet_not = ['регулирование социально-трудовых отношений на основе коллективного договора и соглашений',
        ...
        list_reason_not = ['ничего не знаю о профсоюзах', 'не предлагали вступить', 'препятствия со стороны руководства предприятия',
                           'отсутствие пользы и выгоды от профсоюза', 'нет пересечений видов деятельности с моими интересами',
                           'низкое качество работы профсоюзного комитета и штатных работников ППОО (при наличии)',
                           'недостаточная организация культурно-массовой и спортивной работы', 'недостаточная информационная деятельность',
                           'использование устаревших и несовременных подходов в деятельности профсоюза',
                           'пассивная позиция в решении вопросов с руководством предприятия', 'высокий размер профсоюзного взноса']
        ...
        list_reason_for_joing = ['Ходатайство о повышении зп', 'Плавающий график работы', 'Новые мероприятия на предприятии',
                                 'Улучшение взаимоотношений между руководством и простыми рабочими', 'Условия труда']
        random_reason_for_joing = random.choice(list_reason_for_joing)
        driver.find_element(By.XPATH, '//*[@id="answer_long_text_30903456"]').send_keys(f'{random_reason_for_joing}')

        # choise reason for reluctance on third page
        list_reason_for_reluctance = ['ничего не знают о работе профсоюза', 'не знают, что на предприятии есть первичная профсоюзная организация',
                                      'препятствия со стороны руководства предприятия', 'неуверенность в том, что профсоюз сможет защитить права работников',
                                      'низкий авторитет профсоюза', 'уверенность работников в самостоятельной защите прав',
                                      'отсутствие системной работы по вовлечению в профсоюз', 'нежелание платить профсоюзные взносы',
                                      'использование устаревших и несовременных подходов в деятельности профсоюза', 'непонимание значения и пользы профсоюза',
                                      'иное']
        ...
        list_prioritet = ['регулирование социально-трудовых отношений на основе коллективного договора и соглашений',
                            'представление интересов членов профсоюза на различных уровнях',
                            'решение вопросов достойной заработной платы работников',
                            'оказание правовой помощи и юридической защиты членов профсоюза',
                            'контроль за соблюдением требований охраны труда, улучшением условий труда',
    ...
    list_sources = ['личный контакт', 'печатные издания', 'информационные стенды', 'телевидение, радио на предприятии',
                    'сайты профсоюза и его организаций', 'социальные сети и мессенджеры', 'корпоративные ресурсы (почта, локальный инфопортал предприятия и т.д.)']
